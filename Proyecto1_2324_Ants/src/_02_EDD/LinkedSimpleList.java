...
/**
 * Clase LinkedSimpleList Esta clase define el objeto lista y su comportamiento,
 * con sus atributos pFirst, pLast y size. Siendo pFirst y pLast de la clase
 * SimpleNode.
 *
 * @author AresR
 * @version 17/02/2024
 * @param <A> Representa el tipo de dato que guardara el nodo, declarado
 * general. Se usa "<>" para declararlo generico.
 */
public class LinkedSimpleList<A> implements ILinkedList<A> {
    ...
    /**
     * Instancia la clase LinkedSimpleList definiendo los atributos pFirst y
     ...
     * clase LinkedSimpleList. En caso de que el valor no se encuentre en la
    ...
    public String SummaryOfListOfVertexs() {
        ...
        for (SimpleNode pNode = this.GetpFirst(); pNode != null; pNode = pNode.GetNxt()) {
            if (pNode != this.GetpLast()) {
                Vertex vertexInNode = (Vertex) pNode.GetData();
                txt.append(vertexInNode.GetNumVertex()).append(",");
            } else {
                Vertex vertexInNode = (Vertex) pNode.GetData();
                txt.append(vertexInNode.GetNumVertex()).append(".");
            }
     ...
     * LinkedSimpleList, remplazandolo por un nuevo nodo.
     *
     * @param nFirst
     */
    @Override
